...
The thinking level the active session is actually using, as resolved and reported by the host (`auto` already classified, clamped to the current model), rendered in the widget directly behind the context progress bar. `Effort: ?` marks an unavailable level. The `Effort:` label stays at the terminal's default foreground (matching `Bal:` / `Accrued:`); only the level value is tinted with the theme's per-level thinking colour (`thinkingHigh`, …), falling back to `thinkingOff` for `?`. This is the session's effective level, not the persisted configuration value.
