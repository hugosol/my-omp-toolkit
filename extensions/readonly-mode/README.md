...
所有模式下，每轮对话结束后，Agent 的写操作会自动记录。状态栏常驻显示计数。

```
默认: 折叠为一行 "3 writes this turn  | /readonly audit to expand"
      被拦截的操作标注 "(2 blocked)"
      无操作时显示 "Audit: —"
展开: /readonly audit → 显示操作明细
      文件操作合并显示: "foo.ts (write, edit, ast_edit)  3 ops"
      被拦截条目标注 ✗ blocked
...
3. /readonly audit     → 查看 Agent 做了哪些操作（所有模式可用）
