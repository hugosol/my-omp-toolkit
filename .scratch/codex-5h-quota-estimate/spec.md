...
- **Persisted state** (single human-readable document, revised during the readable-format change):

  ```json
  {
    "baseline": {
      "capturedAt": "2026-09-21T11:23:47.817+08:00",
      "fiveHour": { "usedPercent": 12.4, "resetsAt": "2026-09-21T16:23:47.817+08:00" },
      "weekly": { "usedPercent": 3.1, "resetsAt": "2026-09-24T11:23:47.817+08:00" }
    },
    "ratio": null
  }
  ```

  There is no schema version field: a document that does not match the readable shape — including the previous six-number document — is treated as a first run rather than migrated. Timestamps are local-time ISO-8601 strings carrying their UTC offset, with millisecond precision; measurement numbers are written with four decimals to keep floating-point noise out of the file. The first usable paired reading publishes the baseline with `ratio: null`, so a restart or a long idle stretch continues the same measurement instead of re-anchoring; the ratio is filled in once a sample qualifies and is never downgraded back to null. Load-time validation checks shape and ranges (named baseline windows present, `0 ≤ usedPercent ≤ 100`, parseable timestamps, `ratio` null or `> 0`, capture time not ahead of the local clock); a missing, legacy or invalid document is treated as a first run. The live in-progress sample (current deltas and candidate ratio) is never persisted because it is recomputable from the baseline plus the latest observation.
- **Storage mechanics**: one state file named `codex-usage-estimate.json` in the extension's existing cost-archive directory, written through the extension's cross-process file lock and the existing temp-file + atomic-rename pattern (with the Windows rename retry). Writes are asynchronous and best-effort: a failed write keeps in-memory state and is retried at the next trigger; event handling is never blocked. Concurrent OMP instances publish through the shared lock with a read-compare-write merge: inside the lock the on-disk document is compared by baseline capture time (`at`), and the document with the newer baseline wins, so older state can never overwrite newer state; a within-window baseline written by another instance remains valid because it anchors the same windows. A baseline-only state never downgrades a known ratio: on adoption the newer document's ratio wins when it has one, otherwise the instance's own is kept; on a winning write the instance's ratio wins when present, otherwise the document's.
- **Write policy**: the first usable paired reading and every later baseline change write immediately, with `ratio` null until one has been learned; a qualifying ratio update writes at most once per 60 seconds. No other event writes the file.
